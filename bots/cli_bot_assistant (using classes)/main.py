...
def show_all():
    ...
    if ADDRESS_BOOK.data:
        ...
        for name in ADDRESS_BOOK.data:
            records += f"{name}: {ADDRESS_BOOK[name].get_phones()}\n"
        return records
    else:
...
def incorrect_command():
    ...
    """
    return "No command found. Please type one of this: hello, add [Name] [phone1] [phone2]..., change [Name] [" \
           "old_phone] [new_phone], phone [Name], delete [Name] [phone], show all, exit, good bye, close. "
